...
int rbms::rbms_send(int* motor) {//motorへ制御信号を送信する関数
    ...
    for(int i=0;i<_motor_num;i++){  //int dataを2byteに分割
        if(motor[i]>(_motor_type[i]?16384:10000))return 0;//入力値がmotor上限以上の場合return0
        ...
        for(int id=0;id<_motor_num;id++){
            if(_msg.id==0x201+id){//esc idごとに受信データ割り振り
                ...
                if(_motor_type[id]){
                    ...
                    motor[id] = (int)pid(chrono::duration<float>(tm[id].elapsed_time()).count(),speed[id]/36,set_speed[id],&delta_rpm_pre[id],&ie[id],15,6);
                }
                tm[id].reset();//timer reset
                mot_max=_motor_type[id]?16384:10000;
                if(motor[id]>mot_max)motor[id]=mot_max;
                else if(motor[id]<-mot_max)motor[id]=-mot_max;//上限確認超えてた場合は上限値にset
            }
        }
